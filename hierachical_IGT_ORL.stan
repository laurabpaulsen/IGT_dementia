      ...
      for (deck in 1:C) {
        if (deck == choice[i-1]) {
          ...
          if (outcome[i-1] >= 0) {
            ...
            // negative, loss
            Ev[deck, i] = Ev[deck, i-1] + (a_pun[subject[i]] * (outcome[i-1] - Ev[deck, i-1]));
            Ef[deck, i] = Ef[deck, i-1] + (a_pun[subject[i]] * (sign_out[i-1]) - Ef[deck, i-1]);
          }
        } else {
          ...
          if (outcome[i-1] >= 0) {
            ...
            // negative, loss
            Ef[deck, i] = Ef[deck, i-1] + (a_pun[subject[i]] * ((-sign_out[i-1])/(C-1)) - Ef[deck, i-1]);
          }
        }
      }
      p[,i] = softmax((Ev[,i] + Ef[,i]*omega_f[subject[i]] + PS[,i]*omega_p[subject[i]]));
    }
  }
}
